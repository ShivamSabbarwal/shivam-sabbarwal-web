...
- **Theming**: Custom `ThemeContext` with `localStorage` persistence, favicon swap, and theme-aware project screenshots
...
app/
  ...
  page.tsx                 # Home (Hero, Timeline, TechStack, Projects, Contact, FAQ)
  robots.ts                # Dynamic robots.txt (incl. AI crawler Allows)
  sitemap.ts               # Dynamic sitemap.xml
  opengraph-image.tsx      # 1200×630 branded social card
...
components/
  ...
  sections/                # Hero, Timeline, TechStack, Projects (client)
                           # Faq + Contact are RSC; ContactForm is the client leaf
  resume/                  # Client resume shell + presentational sections
...
lib/
  utils.ts                 # cn() helper
  seo.ts                   # Person graph, FAQ copy, JSON-LD builders
public/
  llms.txt                 # Machine-readable identity digest for AI systems
```
...
| Route       | Purpose                                                                                          |
| ----------- | ------------------------------------------------------------------------------------------------ |
| `/`         | Main portfolio: Hero, Career, Capabilities, Work, Contact, then a quiet FAQ                      |
| `/resume`   | Printable resume; `?variant=leader` selects the leadership version and `?print=true` auto-opens print |
| `/llms.txt` | Short third-person digest for answer engines and AI crawlers                                     |
...
- **Content is data.** Timeline entries, projects, capability practices, tech categories, and resume data live in `constants/content.ts` and are imported directly by the components that render them.
- **Discoverability.** `lib/seo.ts` owns the canonical Person entity, FAQ answers, and JSON-LD graphs (WebSite, WebPage, Person, FAQPage, ProfilePage). The on-page FAQ uses native `<details>` so answers stay in the HTML while staying compact. `public/llms.txt` is the AI-facing digest; keep it aligned with on-page facts. Social previews use the branded `opengraph-image` route, not the square profile photo.
